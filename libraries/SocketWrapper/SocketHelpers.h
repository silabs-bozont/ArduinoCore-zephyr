	...
	static void event_handler(struct net_mgmt_event_callback *cb, uint64_t mgmt_event,
		...
		for (i = 0; i < NET_IF_MAX_IPV4_ADDR; i++) {
			...
			LOG_INF("   Address[%d]: %s", net_if_get_by_iface(iface),
					net_addr_ntop(AF_INET, &iface->config.ip.ipv4->unicast[i].ipv4.address.in_addr,
								  buf, sizeof(buf)));
...
public:
	...
	bool begin(bool blocking = true, uint32_t additional_event_mask = 0) {
		dhcp();
		int ret =
			net_mgmt_event_wait_on_iface(netif, NET_EVENT_IPV4_ADDR_ADD | additional_event_mask,
